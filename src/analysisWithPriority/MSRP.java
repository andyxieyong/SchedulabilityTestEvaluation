...
public class MSRP extends RuntimeCostAnalysis {

	public static boolean btbhit = false;
	public static boolean useri = false;
	public static int setOne = 1;
	...
	public long[][] getResponseTimeSBPO(ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, boolean isprint) {
		...
		for (int i = 0; i < tasks.size(); i++) {
			...
			for (int currentLevel = 0; currentLevel < prioLevels; currentLevel++) {
				...
				for (int j = startingIndex; j >= 0; j--) {
					...
					while (!isEqual) {
						...
						dummy_response_time_plus = getResponseTimeForSBPO(task.partition, tasks, resources, btbhit, extendCal, dummy_response_time, task);
			...
			while (!isEqual) {
				...
				dummy_response_time_plus = getResponseTimeForOnePartition(partition, tasks, resources, btbhit, 1, dummy_response_time);
		...
		long[][] response_time = initResponseTime(tasks);
		...
		for (int i = 0; i < tasks.size(); i++) {
			...
			for (int currentLevel = 0; currentLevel < prioLevels; currentLevel++) {
				...
				for (int j = startingIndex; j >= 0; j--) {
					...
					tasks.get(i).sort((t1, t2) -> -Integer.compare(t1.priority, t2.priority));
					long time = getResponseTimeForOneTask(task, tasks, resources, btbhit);
					task.priority = originalP;
					tasks.get(i).sort((t1, t2) -> -Integer.compare(t1.priority, t2.priority));
	...
	private long[] getResponseTimeForOnePartition(int partition, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, boolean btbHit,
		...
		for (int i = 0; i < tasks.get(partition).size(); i++) {
			...
			task.spin = directRemoteDelay(task, tasks, resources, response_time, response_time[partition][i], btbHit, useri);
			task.interference = highPriorityInterference(task, tasks, resources, response_time, response_time[partition][i], btbHit, useri);
			task.local = localBlocking(task, tasks, resources, response_time, response_time[partition][i], btbHit, useri);
			response_time_plus[i] = task.Ri = task.WCET + task.spin + task.interference + task.local;
		}
				...
				for (int j = 0; j < tasks.get(i).size(); j++) {
					if (tasks.get(i).get(j).resource_required_index.contains(resource.id - 1)) {
						...
						int number_of_release = (int) Math
								.ceil((double) (Ri + (btbHit ? (useRi ? Ris[i][j] : remote_task.deadline) : 0)) / (double) remote_task.period) + setOne;
		...
		for (int i = 0; i < tasks.size(); i++) {
			if (tasks.get(i).resource_required_index.contains(resource.id - 1)) {
				...
				number_of_request_by_Remote_P += (Math
						.ceil((double) (Ri + (btbHit ? (useRi ? Ris[i] : remote_task.deadline) : 0)) / (double) remote_task.period) + setOne)
