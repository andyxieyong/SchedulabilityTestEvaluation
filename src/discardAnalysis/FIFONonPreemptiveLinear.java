...
//			for (int j = 0; j < tasks.get(i).size(); j++) {
//				SporadicTask task = tasks.get(i).get(j);
//				generateFixedConstrains(task, tasks, resources, response_time, response_time[i][j], printLIP);
//			}
//		}
//
...
//		if (printBebug) {
//			if (missDeadline)
//				System.out.println("LIP-FIFO-NP    after " + count + " tims of recursion, the tasks miss the deadline.");
//			else
//				System.out.println("LIP-FIFO-NP    after " + count + " tims of recursion, we got the response time.");
...
//
//				response_time_plus[i][j] = task.Ri = task.WCET + task.pure_resource_execution_time + task.interference + task.spin
//						+ task.local;
...
//
//	/*
//	 * Calculate the local high priority tasks' interference for a given task t. CI is a set of computation time of local tasks, including
//	 * spin delay.
//	 */
...
//	 ******************************************/
//	public void generateFixedConstrains(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources,
...
//	}
//
//	public double[] solveMILPForOneTask(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources,
//			long[][] Ris, long Ri, boolean printDebug) {
...
//	}
//
//	public void generateObjectiveandC1C2C5C6(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources) {
...
//									(other_task.id - 1) + "|" + other_task.resource_required_index.get(k) + "|" + l + "|" + "SD");
//							int variable2 = addGetVariable(task.solver.variables,
//									(other_task.id - 1) + "|" + other_task.resource_required_index.get(k) + "|" + l + "|" + "LB");
//
...
//	}
//
//	public void c2c3c4(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources) {
...
//			int variable_index = addGetVariable(task.solver.variables, 0 + "|" + (resource.id - 1) + "|" + 0 + "|" + "PR");
//
...
//	}
//
//	public void constrainSeven(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources) {
//		for (int i = 0; i < resources.size(); i++) {
...
//	}
//
//	public void constrainNine(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources) {
//
//		for (int i = 0; i < tasks.size(); i++) {
//			if (i == task.partition)
//				continue;
...
//	}
//
//	public void constrainEight(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] Ris,
//			long Ri) {
//
//		for (int i = 0; i < resources.size(); i++) {
//			Resource resource = resources.get(i);
...
//			int ncs = getNoRFromHP(resource, task, tasks.get(task.partition), Ris[task.partition], Ri) + (requested_by_Task
...
//	}
//
//	public ArrayList<Integer> getNewValues(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources,
//			long[][] Ris, long Ri) {
...
//				System.out.println("variable index: " + i + " variable: " + task.solver.variables.get(i));
//			}
//
//			System.out.println("binrays: " + task.solver.binary_index.size());
//			for (int i = 0; i < task.solver.binary_index.size(); i++) {
//				System.out.println("variable index: " + task.solver.binary_index.get(i) + " variable: "
...
//				System.out.print(task.solver.coefficient.get(i) + " * " + task.solver.objective.get(i) + "      ");
//			}
//			System.out.println();
//
